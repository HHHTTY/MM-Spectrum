...
def model_opts(parser):
    ...
    # === Encoder MoE options ===
    group = parser.add_argument_group('Encoder-MoE')
    group.add_argument('--enc_num_experts', type=int, default=0,
                   help='Number of MoE experts in encoder FFN (0=disable).')
    group.add_argument('--enc_num_experts_per_tok', type=int, default=2,
                   help='Top-k experts per token for encoder MoE.')
    group.add_argument('--add_ffnbias', action='store_true',
                   help='Add bias to encoder FFN/MoE-FFN linears.')

    # NEW: explicit modality-aware routing (router bias by modality id)
    group.add_argument(
        '--enc_modality_aware_routing',
        action='store_true',
        help=(
            'Enable explicit modality-aware routing for encoder MoE. '
            'If enabled, the router logits use: logits = W h + b_modality, '
            'where b_modality is a learned bias vector per modality id.'
        ),
    )
    group.add_argument(
        '--enc_num_modalities',
        ...
        default=0,
        help=(
            'Number of modality ids (size of b_modality embedding). '
            'Required when enc_modality_aware_routing is enabled. '
            'Example: for N/I/M, set enc_num_modalities=3.'
        ),
    )

    # NEW: heterogeneous experts (per-expert FFN widths) and compute-aware regularization
    group.add_argument(
        '--enc_expert_dffs',
        type=str,
        default=None,
        help=(
            'Per-expert FFN widths (heterogeneous experts). '
            'Accepts YAML list string, e.g. "[2048,2048,4096,4096]", '
            'or CSV, e.g. "2048,2048,4096,4096". '
            'Length must equal enc_num_experts.'
        ),
    )
    group.add_argument(
        '--enc_comp_beta',
        ...
        default=0.0,
        help=(
            'Compute-aware regularization weight (beta). '
            'Adds expected expert compute cost under router probs as an aux loss. '
            'Use >0 together with enc_expert_dffs to prevent routing collapsing to large experts.'
        ),
    )

    # === Encoder-MoE (routing controls) ===
    group = parser.add_argument_group('Encoder-MoE Router')
    group.add_argument('--enc_router_temp', type=float, default=1.0,
                   help='Gating temperature; <1 makes routing sharper.')
    group.add_argument('--enc_router_noise_std', type=float, default=0.0,
                   help='Add N(0, std^2) noise to router logits during training.')

    group.add_argument('--enc_aux_loss_alpha', type=float, default=0.02,
                   help='Coefficient for MoE load-balance auxiliary loss')

    # === Encoder-MoE (aux alpha scheduling) ===
    group = parser.add_argument_group('Encoder-MoE Aux-Alpha Schedule')
    group.add_argument('--enc_aux_alpha_max', type=float, default=None,
                   help='(MoE) initial/max aux-balance loss weight alpha; enable scheduling only when both max & min are set.')
    group.add_argument('--enc_aux_alpha_min', type=float, default=None,
                   help='(MoE) final/min aux-balance loss weight alpha; enable scheduling only when both max & min are set.')
    group.add_argument('--enc_aux_alpha_warmup_steps', type=int, default=0,
                   help='(MoE) keep alpha at max for these steps, then start annealing.')
    group.add_argument('--enc_aux_alpha_schedule', type=str, default="cosine",
                   choices=["cosine", "linear", "none"],
                   help='(MoE) schedule shape for aux alpha.')
    group.add_argument('--enc_aux_alpha_log_every', type=int, default=1000,
                   help='(MoE) log current aux alpha every N steps.')
    group.add_argument('--enc_aux_alpha_hold_last_steps', type=int, default=0,
                   help='(MoE) keep alpha=min for the last K steps; often useful to let experts fully specialize near the end.')

    # === Encoder-MoE (router noise scheduling) ===
    group = parser.add_argument_group('Encoder-MoE Router Noise Schedule')
    group.add_argument('--enc_router_noise_max', type=float, default=None,
                   help='(MoE) initial/max router noise std; enable scheduling only when both max & min are set.')
    group.add_argument('--enc_router_noise_min', type=float, default=None,
                   help='(MoE) final/min router noise std; enable scheduling only when both max & min are set.')
    group.add_argument('--enc_router_noise_warmup_steps', type=int, default=0,
                   help='(MoE) keep sigma at max for these steps, then start annealing.')
    group.add_argument('--enc_router_noise_schedule', type=str, default="cosine",
                   choices=["cosine", "linear", "none"],
                   help='(MoE) schedule shape for router noise.')
    group.add_argument('--enc_router_noise_log_every', type=int, default=1000,
                   help='(MoE) log current router noise std every N steps.')
    group.add_argument('--enc_router_noise_hold_last_steps', type=int, default=0,
                   help='(MoE) keep sigma=min for the last K steps to reduce train/test routing mismatch.')

    # === Encoder-MoE (joint scheduling) ===
    group = parser.add_argument_group('Encoder-MoE Joint Schedule')
    group.add_argument('--enc_moe_joint_schedule', type=str, default="off",
        choices=["off", "decoupled", "coupled_direct", "coupled_inverse"],
        help="off=不用联合函数；decoupled=同一函数同时算两条独立曲线；"
         "coupled_direct=alpha 与 noise 同向联动；"
         "coupled_inverse=反向联动。")
    group.add_argument('--enc_moe_joint_log_every', type=int, default=1000,
        help='log 当前 alpha/sigma 每 N 步（联合模式下）。')
    # 联动强度（仅当 coupled_* 模式时生效）
    group.add_argument('--enc_moe_couple_gamma', type=float, default=0.7,
        help='0..1，alpha(t)=time驱动*(1-gamma)+noise驱动*gamma，'
         'direct=用 s_noise；inverse=用(1-s_noise)。')
