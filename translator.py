...
class Translator(Inference):
    ...
    def _run_encoder(self, batch):
        ...
        # MoE interpretability logging during inference/translation (JSONL)
        if os.environ.get("MOE_LOG", "0") == "1":
            try:
                if not hasattr(self, "_moe_logger"):
                    from onmt.utils.moe_logger import MoELogger
                    self._moe_logger = MoELogger.from_env(opt=None, gpu_rank=0)
                step_i = self._moe_logger.next_infer_step()
                self._moe_logger.log(step=step_i, split="infer", model=self.model,
